...
$i = $_POST;
  if(isset($i['submit']) ){
    ...
    if(!$photo){
      $update = "UPDATE tb_users SET name='$name', username='$username', password='$password', biodata='$biodata', level='$level' WHERE id='$id' ";
      ...
      $moved = move_uploaded_file($sumber, $dir.$photo);
        if($moved){
          $update = "UPDATE tb_users SET name='$name', username='$username', password='$password', 	photoProfile='$photo', biodata='$biodata', level='$level' WHERE id='$id' ";
          $update_result = $conn->query($update);
                if($update_result){
                  echo "<script>alert('Data Berhasil di Update');</script>";
